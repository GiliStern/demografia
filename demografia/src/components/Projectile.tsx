...
export const Projectile = ({ position, velocity, duration, damage, spriteConfig, onDespawn }: ProjectileProps) => {
  ...
  useEffect(() => {
    ...
    if (rigidBody.current) {
       rigidBody.current.setLinvel({ x: velocity[0], y: velocity[1], z: 0 }, true);
    }

    // Despawn timer
    timeoutRef.current = setTimeout(onDespawn, duration * 1000) as unknown as number;
  ...
  const handleIntersection = (payload: any) => {
     ...
     if (payload.other.rigidBodyObject?.userData && (payload.other.rigidBodyObject.userData as any).type === 'enemy') {
